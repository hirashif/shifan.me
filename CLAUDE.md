...
- **/outbound/** — a case study ("How I ran outbound for my own job search"). It started as a noindex, email-only page; shifan has since made it **public like the writing posts**: no noindex, in the sitemap, and listed first in the home page's writing section as "running outbound on my own job search" (`sep`). It is **not** a writing-collection post and must stay at `/outbound/` — that url is already out in emails, so it joins the home list via the `standalone` array in `src/pages/index.astro`, and it does not appear on `/writing`. Not in the dock. The text is his, rendered verbatim from `src/content/outbound.md` — edit it there, never retype or rewrite it. It is **deliberately sentence case**, a standing exception to the lowercase rule, and deliberately anonymized: add no company or person names. The funnel table stacks into label/value rows under 640px. Body styles come from the shared `src/styles/prose.css`, the same file the writing posts use.
...
- **The resume is linked from the dock, not from page content.** Work history lives
  ...
  **Second resume variant: `/hereismyresume/ai/`** (PDF at `/hereismyresume-ai.pdf`,
  the ai/gtm version). Same page, same noindex meta, same `X-Robots-Tag`, excluded
  from the sitemap by the same filter — but deliberately **not** linked from the dock.
  shifan hands it out directly. Keep the two pages identical apart from the PDF path.
  The `_headers` rule `/hereismyresume/*` is what covers both pages' real (trailing-
  slash) URLs; the bare `/hereismyresume` rule only ever matches the 307 redirect.

  **Updating the resumes: `pnpm resumes`** (`scripts/update-resumes.sh`). It copies
  `Shifan_Hirani_Resume_web.pdf` and `..._GTM_web.pdf` from `~/Documents/job apps/resumes/`,
  commits and deploys shifan.me if either changed, updates `hirashif.github.io/resume.pdf`
  to match the main one (that old URL is still publicly indexed, so it must not go stale),
  then checks all three live URLs by md5. It refuses to run off `main` or with other
  uncommitted changes, because `pnpm build` would deploy those too. Use it rather than
  copying PDFs by hand, which is how the github.io copy drifted before.
...
- **Token spend:** `GET /api/tokens` reads a snapshot from Cloudflare **KV**. It is NOT the Anthropic cost API — shifan is on a Claude Code subscription, which that API cannot see. An hourly launchd job at `~/.local/bin/shifan-usage-push.sh` runs `usage-snapshot.py` (stdlib Python, sits next to it) over `~/.claude/projects/**/*.jsonl` and POSTs to `/api/usage`. Both files are copies of `scripts/usage-cron.sh` and `scripts/usage-snapshot.py`; edit in the repo, then `cp` both over. The scripts deliberately live outside `~/Documents`: a LaunchAgent cannot read it there without Full Disk Access (macOS TCC), which broke the first version.
  **The aggregation mirrors T3 Code's usage panel** so the footer and that panel agree to the cent: dedupe on `message.id`, LiteLLM's live rate table (cached at `~/.config/shifan/litellm-rates.json`), local-day buckets, subagent logs included. The card's three rows are today / this week / **last 3 months** (`quarter`, a rolling 90 days, T3's default window). It used to be "this year", but Claude Code prunes transcripts (`cleanupPeriodDays`, default 30) so the logs never reach back a year; `~/.claude/settings.json` now sets it to 120 so the 90-day window is always fully covered. It replaced `ccusage`, whose bundled offline rate table had no `claude-fable-5-1` entry and priced every session on that model at $0 — the footer sat ~30% under T3 as a result. If a new model shows up unpriced, the script warns on stderr into `/tmp/shifan-usage-push.log`; the fix is upstream in LiteLLM, not here.
